        ...
        {toc.map(({ id, content, type, offsetTop }, index) => {
          return (
                ...
                <Flex
                  ...
                  pb={"2px"}
                  pt={"2px"}
                  bg={id === activeItemId ? "#D3F3FA" : "FFFFFF"}
                  borderStyle="solid"
                  borderTopWidth="0px"
                  borderBottomWidth={"0px"}
                  borderRightWidth="0px"
                  borderLeftWidth={
                    id === activeItemId ? `${BORDER_WIDTH}px` : "0px"
                  }
                  borderLeftColor={id === activeItemId ? "#0990D0" : "#F5F5F5"}
                  mb={type === 1 ? "10px" : "0px"}
                  maxW="200px"
                >
                  <AnchorLink href={`#${id}`} offset="120" key={id} color="#333333">
                    <Flex color={id === activeItemId ? "rgba(0, 0, 0, 0.87)": "rgba(0, 0, 0, 0.6)"}>
